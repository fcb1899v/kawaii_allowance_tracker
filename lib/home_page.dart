...
class MyHomePage extends HookConsumerWidget {
  ...
  Widget build(BuildContext context, WidgetRef ref) {
    ...
    onChangeInitialAssets(String value) async {
      "value: $value".debugPrint();
      isInput.value = await value.setIsInput(false);
      inputInitial.value = await value.setInputAmount(isInput.value, false, unit.value);
      isInitialInput.value = await value.setIsInitialAssetsInput(isInput.value);
    }
    ...
    onChangeItem(String value, bool isSpend) async {
      "value: $value".debugPrint();
      isInput.value = await value.isNotEmpty;
      inputItem.value = await value.setInputItem(context, isInput.value, isSpend);
      isItemInput.value = await value.setIsItemInput(isInput.value, isSpend);
    }
    ...
    onChangeAmount(String value, bool isSpend) async {
      "value: $value".debugPrint();
      isInput.value = await value.setIsInput(false);
      inputAmount.value = await value.setInputAmount(isInput.value, isSpend, unit.value);
      isAmountInput.value = await value.setIsAmountInput(isInput.value);
    }
